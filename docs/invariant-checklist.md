...
| Type | Where | What it makes impossible |
| --- | --- | --- |
| `PoisonFlag` and `MutationPermit` (round 12, **tightened round 14**) | declared in `crates/varve-core/src/writer_permit.rs`; used by `stream.rs`, `indexed.rs`, `file.rs`, `layout.rs` | `PoisonFlag` is the only holder of the boolean; `MutationPermit` has a private field and no constructor other than `PoisonFlag::issue`, and the module boundary makes the field unnameable even inside the file that owns the writer. **Round 14 closed a laundering bypass**: `issue` was `pub(crate)` and `PoisonFlag::healthy()` was a `const fn`, so three lines anywhere in the crate could mint a witness from a throwaway flag and spend it on a genuinely poisoned writer — the permit proved that *a* flag had been checked, not that *this writer's* had. `issue` now has no visibility modifier at all, so the only route out of the module is `GuardedWriter::writer_permit`, which takes `&self` of the writer and reads that writer's own flag; and the witness is typed by the writer it speaks for (`MutationPermit<VarveStreamWriter>` is not `MutationPermit<VarveFile>`). **Round 15 narrowed the decoy-flag route, and does not claim it closed**: `PoisonFlag::healthy()` is no longer a `const fn`, so `static DECOY: PoisonFlag = PoisonFlag::healthy();` — a value a writer's own `poison_flag()` could return in place of its real field, permitting every guarded operation on a poisoned writer — no longer compiles (E0015, pinned by `writer_permit.rs::tests::a_static_decoy_flag_cannot_be_declared`). The leaked-heap spelling `Box::leak(Box::new(PoisonFlag::healthy()))` still compiles and is caught only by a source gate on the four `poison_flag` impls; see open item 27. *Instance* identity is still unenforced — see open item 21. Every guarded operation takes a permit, by value where another module can reach it, so each such mutation is preceded by its own fresh check rather than by one check amortised over a loop. A caller that skips the check has nothing to pass and does not compile. Proved by `crates/varve/tests/ui/fail_fabricated_mutation_permit.rs` (the witness cannot be forged), `crates/varve/tests/ui/fail_cross_writer_mutation_permit.rs` (one writer's permit is not another's) and `crates/varve/tests/enforcement_gates.rs::a_permit_can_only_be_minted_from_the_writers_own_poison_flag` (the constructor stays private; the in-crate half of the bypass cannot be a `tests/ui` fixture, because every spelling of it is already unreachable from a downstream crate and the fixture would pass for the wrong reason) |
...
| `FatalAccessGate` / `FatalAccessAllowed` (**round 14**) | `crates/varve-core/src/matrix.rs`, `mod fatal_access` | The flag the shape-B inventory could not have found: round 12 built that inventory by grepping the crate for `poisoned`, and this one is called `fatal_access_blocked`. It records that a `Fatal` recovery finding blocks safe access to matrix state unless the spec opted into forensic reading, and it was consulted by an `ensure_fatal_access_allowed` helper at eleven call sites — a guard that protects the operations which remember to call it, and nothing else. The boolean is now a private field of `FatalAccessGate`, whose only output is `FatalAccessAllowed`: zero-sized, private field. **Round 15 sealed the minting half.** Round 14 left two constructors that took the answer rather than deriving it: `FatalAccessGate::new(blocked: bool)` accepted the classification, and the method `FatalAccessGate::allow(&self)` turned any gate into the witness — so `FatalAccessGate::new(false).allow()` compiled. Both are deleted. The gate's only constructor is now `fatal_access::evaluate(findings, spec)`, which *performs* the classification inside `mod fatal_access`, and the witness's only producer is the free function `fatal_access::allow_for(layout: &MatrixLayout)` — so the gate consulted is the gate of the state the caller is about to address, which *is* the refusal. `MatrixLayout::block_index` and `MatrixLayout::commit_index` — the only two routes from a block id or a commit category to a position in the layout's state — demand the witness, so an accessor written next month cannot address what it wants to touch without the check. **Stated rather than implied:** code that already holds a resolved position can still index `blocks`/`commits` directly (that is how the existing prepare/commit pairs pass an index between their halves); the enforced claim is that no path can go from an *identifier* to matrix state without the refusal running. Bound: one `bool` per layout, no allocation. Proved by `crates/varve/tests/ui/fail_fabricated_fatal_access.rs`, `crates/varve/tests/ui/fail_minted_fatal_access_gate.rs` and `crates/varve/tests/enforcement_gates.rs::matrix_state_is_only_addressed_through_the_fatal_access_witness`. **Round 16 closed the assignment half.** Round 15 reported its residual as "a twelve-field `MatrixLayout` literal"; re-verification measured it at **one line**, because `MatrixLayout`'s fields are private to the *file*, not to a module: with any `&mut MatrixLayout` in scope, `layout.fatal_access = FatalAccessGate::evaluate(clean.iter(), spec);` installed an unblocked gate and **undid `FatalAccessGate::block()`**, the irreversible poison an interrupted rebuild leaves. `layout.fatal_access = donor.fatal_access.clone();` did the same off a healthy layout and had never been written down at all. The enforced property is no longer a call-site count but a structural one: **no expression of type `FatalAccessGate` exists outside `mod fatal_access`.** `evaluate` is private to the module (E0624 from file scope) and its only caller is `fatal_access::assemble_layout`, which returns the whole layout it derived the gate for; the gate is **not `Clone`** (E0599), so `MatrixLayout::clone` is hand-written through `fatal_access::clone_layout`, which copies a layout and never a decision; and `MatrixLayout` implements an **empty `Drop`**, whose only purpose is E0509 — a gate cannot be *moved* out of `donor.clone().fatal_access` or `assemble_layout(..).fatal_access` either. All five spellings were compiled and observed to fail, and are quoted with their diagnostics in `matrix.rs::bypass_catalogue` entries (10)-(11c). **Residual, measured:** a caller inside `matrix.rs` can still call `assemble_layout` and get an unblocked gate on a layout it built itself out of blocks and commits it supplied — i.e. it must supply the state it wants to address. Field-level unnameability still wants open item 25 |
| `CrcValidEvidence` / `CompleteCrcValidEvidence` (round 12 as a tracked boolean, **restructured round 14**) | `crates/varve-core/src/matrix.rs`, `mod crc_valid_evidence` | A block's CRC-validity bitmap answers "is the stored CRC for this cell meaningful", and a **clear** bit means either "nothing established one" or "the page carrying it was never loaded, because the validity page index was damaged". F-04 was the commit-map rebuild publishing the second as the first. Round 12 fixed it with a `crc_valid_complete: bool` beside the bitmap plus one added check; re-verification judged that **partial**, and compiled a new consumer that read `block.crc_valid_bits.get(ordinal)?` without it. The bitmap is now a private field of `CrcValidEvidence` and there is **no operation outside the module that yields a validity bit** except `CompleteCrcValidEvidence::get`, whose witness is produced solely by `CrcValidEvidence::complete` — which *is* the refusal (`Error::MatrixFatalCorruption`, unconditional; forensic mode does not relax it). The one reader allowed on incomplete evidence, `require_meaningful`, returns `Result<()>`: it hands back no bit, so an absent one can become a refusal but never published state. **Round 15 sealed the minting half.** Round 14 left `CrcValidEvidence::new(bits, complete: bool)`, which *accepted* completeness as a boolean argument, so anyone who could name it could assert a derived fact they had not derived. It is deleted. Completeness is now only expressible by moving in a `PageIndexEnumeration` — `CrcValidEvidence::from_page_index_enumeration(bits, enumeration)` — whose own two constructors carry **no visibility modifier at all** and live in `mod page_index_enumeration` beside the enumerator `load_page_index` that produces them. That termination is the point: wrapping a derived fact in a type does not make it unforgeable, because whoever may *state* it may *restate* it; the chain has to end at the code that derives it. The one remaining constructor, `newly_created(bit_count)`, takes a count and builds its own empty bitmap, so it cannot launder someone else's. **Round 16 closed the escape.** Re-verification showed that the module's one named escape, `page_index_mirror_mut`, returned `&mut SparseBitmap` and was held to a single call site by a source gate — which says nothing about what that one call site may do. It **executed** `let mut ev = CrcValidEvidence::newly_created(n)?; *ev.page_index_mirror_mut() = attacker_bits;` and read a bit no enumeration had produced back through `CompleteCrcValidEvidence::get`: whole-map replacement leaves `complete` true, so `newly_created` laundered someone else's bitmap after all, one method away. The escape now returns `page_index::PageIndexMirror`, whose `&mut SparseBitmap` is a private field of `mod page_index`; outside that module the mirror exposes only the two questions page-index maintenance asks (`indexes_page`, `page_ones_after`). Assignment through it is E0614, borrowing the bitmap back out is E0308, and `set`/`get` on it are E0599 — all compiled and quoted in `matrix.rs::bypass_catalogue` entries (12)-(13b). Installing a fabricated bitmap as complete evidence is refused separately (E0624 on `PageIndexEnumeration::fully_enumerated`, entry (14)). Bound: one `bool` per block, no content-sized allocation. Proved by `crates/varve/tests/ui/fail_fabricated_crc_valid_completeness.rs`, `crates/varve/tests/ui/fail_minted_crc_valid_completeness.rs` and `crates/varve/tests/enforcement_gates.rs::crc_validity_completeness_can_only_come_from_an_enumeration_that_ran` |
...
- `MatrixBlockLayout::crc_valid_bits` stays a `CrcValidEvidence` rather than a
  ...
  laundering of the fail-closed result (round 14). Round 16 added the part the
  count could not express: `page_index_mirror_mut` must return
  `page_index::PageIndexMirror` rather than `&mut SparseBitmap`, and that
  wrapper must keep its borrow private and hand it back through no method — a
  single call site with a raw `&mut` was a whole-map laundering route, and
  re-verification ran it.
...
25. **`MatrixLayout` is not behind a module boundary**, so its fields are
   nameable from all of `matrix.rs`. **Narrowed in round 16 for the one field
   that carries a security decision, not closed for the rest.** The gate field
   can no longer be written (there is no expression of type `FatalAccessGate`
   outside `mod fatal_access`, and `MatrixLayout`'s empty `Drop` impl refuses
   every partial move that would produce one), but every *other* field is still
   assignable from any of the file's ~8000 lines. The one with teeth is
   `layout.commits[i].bits = attacker_bits;`, which compiles today and publishes
   a commit view nothing committed — the same shape as the CRC-validity
   laundering round 16 closed, one level out, and the reason open item 28 exists.
   Roughly seventy call sites index `blocks`/`commits` directly, so this is its
   own round.

26. **`SparseBitmap` and its prepared values are declared at file scope**, so
   `BitmapByteUpdate`, `CommitBitUpdate`, `PreparedByteWrite` and
   `PreparedWriteBit` are all constructible by literal from anywhere in
   `matrix.rs` — a fabricated `PreparedByteWrite { fresh: None, .. }` reaches
   `commit_byte_write` for a non-resident page. This is the same defect shape as
   the sealed gates, one level down: nothing here proves a *check* ran. Closing
   it means a `mod sparse_bitmap`, which is a large refactor of the hot bitmap
   path and needs its own performance check. Catalogued with the exact spellings
   in `matrix.rs::bypass_catalogue`.

27. **The leaked-flag route to a `MutationPermit` still compiles.**
   `Box::leak(Box::new(PoisonFlag::healthy()))` yields a `&'static PoisonFlag`
   that a `GuardedWriter::poison_flag` impl could return instead of the writer's
   own field, after which every guarded operation is permitted on a poisoned
   writer. Round 15 removed the `static DECOY` spelling by de-`const`ing
   `healthy()`; the heap spelling is caught only by a source gate over the four
   `poison_flag` impls (one of which is in `layout.rs`). It wants the same fix
   as open item 21 — the writer's fields split into a borrow-disjoint inner
   struct so the flag cannot be substituted at all.
   Re-confirmed by compilation in round 16: `Box::leak(Box::new(
   PoisonFlag::healthy()))` still builds from `matrix.rs`, i.e. from any module
   of `varve-core`.

28. **Commit maps are not behind an evidence type.** `MatrixCommitLayout::bits`
   is a plain `SparseBitmap` field of a file-scope struct, so
   `layout.commits[i].bits = attacker_bits;` compiles from anywhere in
   `matrix.rs` (measured round 16). Committed-ness is the matrix's published
   visibility, so this is the same class as F-04 — a fact a reader consumes as
   proof — with nothing between the fact and the assignment. The CRC-validity
   half was closed by wrapping the bitmap in `CrcValidEvidence`; the commit half
   needs either the same treatment or open items 25 and 26, and the two should
   be designed together rather than one bitmap at a time.

29. **`ResidentIndex::adopt_generation` is still unchecked**, and round 16
   measured why the cheap fix does not work rather than applying it. The
   constructor takes a `Vec<RecordIndexEntry>` and adopts it wholesale, so
   `adopt_generation(vec![RecordIndexEntry { committed: true, .. }])` compiles
   and `len() == 1` for a record the disk does not have; the justification in
   the shape-A table ("the rewrite paths hand over a `Vec` that already
   describes records on disk") is a statement about today's eight call sites,
   not a property the constructor enforces. The obvious checked replacement —
   take the backing `&File`, read its length, and run each entry through the
   existing `RecordIndexEntry::validate_payload_extent` — **does not refuse the
   example above**: a zero-length entry against a zero-length file passes the
   extent check. A constructor that would refuse it has to read the record
   header at each entry's `record_offset`, which adds O(records) I/O to open for
   every `IndexPolicy` that does not already scan. That is a durability and
   open-cost decision with its own performance budget, and two of the eight call
   sites run *after* the replacement generation is published, where a new
   fallible step has to be folded into the existing `PublishedButRebindFailed`
   treatment. Its own round.
30. **A lazy open is `O(1)` in bytes read only up to the persisted page index,
   which is still `O(live pages)`** (`crates/varve-core/src/matrix.rs`,
   `MatrixMetadataResidency::Lazy`). Measured, not argued: 32 bytes for one live
   page and 1,040 bytes for 64 — an 8-byte entry each, against the 4,096-byte
   page the eager path reads for the same entry, so the term is 512x smaller but
   it is not gone. At a million live pages it is 8 MB of index read at open. The
   honest statement of the criterion is therefore "open reads no *page payload*
   and queries no allocation map", which is what
   `matrix_lazy_residency.rs::a_lazy_open_does_not_read_a_page_per_live_page`
   asserts (a 64x live-page range must not cost a page more). Making the index
   itself demand-paged would need it to be searchable on disk rather than
   enumerated into a `HashMap`, and would put the F-06 "unpublished is not
   uncached" distinction — which currently rests on the index being complete in
   memory — behind an I/O. Not attempted; it is a different structure, not a
   tuning change.
31. **There is now one lock in the matrix read path, where round 16 had none.**
   `SparseBitmap` holds a `Mutex<PageStore>` so a demand fault-in can happen
   under `&self`. It is narrow — taken only for `O(1)` map operations, released
   for the whole of the fault-in read (`SparseBitmap::faulted_store`), and never
   taken at all on the write path, which reaches the store through
   `Mutex::get_mut` — but it is a shared point that four threads reading one
   category touch on every read, and under `EagerVerified` (the default) it buys
   nothing, because nothing is ever faulted in. The reason it is recorded rather
   than removed: the alternative is `RwLock` (worse for a workload whose reads
   all take the write side on a miss) or a lock-free page map (an unbounded
   design change). It is held honest by wall-clock scaling contracts on both
   policies — `matrix_concurrent_reads.rs::one_shared_handle_does_not_serialise_concurrent_readers`
   and `matrix_lazy_residency.rs::concurrent_lazy_readers_are_not_serialised_behind_the_page_store`
   — and the second of those *caught a real convoy while being written*: the
   fault-in read originally went through the single duplicated descriptor in
   `LazyResidency`, which made four threads 1.54x **slower** than one on
   Windows, because `ReadFile` serialises on the kernel file object. Routing it
   through the same per-thread `MatrixReadPool` the eager path uses took it to
   0.43x. Any future lock added here must arrive with the same measurement; an
   occupancy counter cannot see this class of defect, which is why round 16
   replaced the one it had.
