...
- Append-log readers are snapshot-on-open. Live tailing is out of scope in 0.3. A snapshot retains the opened object and validated logical EOF; it does
  ...
  generations, or read leases that are outside VMAT v4.
...
- Valid native 0.1 wire contracts remain readable in 0.3. Wire changes on the
  unreleased branch (computed schema hash v3, matrix `VMAT` v4 with `MCRC` v2,
...
- Matrix/preallocated storage now has a P0 implementation for dense direct
  ...
  hole where the platform supports range removal and otherwise streams zeros; a
  sparse page whose last set bit is cleared is evicted and refunded rather than
  kept resident, and its page-index entry is released with it. That change is
  `VMAT` layout version 4 (with the `MCRC` integrity table at version 2);
  version 1, 2, and 3 matrix files are refused with `FormatVersionMismatch`.
