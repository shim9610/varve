use varve::{
    ...
    VarveIndexedWriter, VarveKeyedBlock, WireType,
};
...
impl VarveBlock for ContradictoryKeyed {
    ...
    const SCHEMA_FINGERPRINT: u64 = 0x436f_6e74_7261_6434;
}
...
fn low_level_delete(writer: &mut VarveIndexedWriter) -> Result<AppendInfo> {
    writer.delete_unindexed_info::<ContradictoryKeyed>(&0u64)
}

fn main() {
    // Reify the low-level indexed delete path exactly like a real caller
    // would; the KeyedBlockContract const inside delete_unindexed_info then
    // fails to evaluate at monomorphization time (API2-03).
    let _ = low_level_delete as fn(&mut VarveIndexedWriter) -> Result<AppendInfo>;
}
